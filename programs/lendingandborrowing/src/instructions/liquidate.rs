...
pub fn process_liquidate(ctx: Context<Liquidate>) -> Result<()> {
  ...
  let user = &mut ctx.accounts.liquidator_user_account;
  ...
  match ctx.accounts.collateral_mint.to_account_info().key() {
    key if key == user.usdc_address => {
      ...
      let new_sol = calculate_accrued_interest(user.deposited_sol, collateral_bank.interest_rate, user.last_updated_borrow)?;
    ...
    _ => {
      ...
      let new_usdc = calculate_accrued_interest(user.deposited_usdc, collateral_bank.interest_rate, user.last_updated_borrow)?;
  ...
  if health_factor >= 1.0 {
    return Err(error!(ErrorCode::NotUnderCollateralized));
  }
  ...
  let liquidator_bonus = (liquidation_amount * collateral_bank.liquidation_bonus) + liquidation_amount;
  ...
  token_interface::transfer_checked(cpi_ctx_to_liquidator, liquidator_bonus, collateral_decimals)?;
